...
def _vector_to_angles(vector, defaults={}):
    ...
    if not (ax or ay or az):
        return 0.0, 0.0
    pitch = math.degrees(math.atan2(-ax, math.hypot(ay, az))) - defaults.get(
        "base_pitch", 0.0
    )
...
def _vector_angle(
    v0, defaults={}
):  # 5$ if you can manage to get a divide by 0, not guarding it.
    v1 = defaults.get("base_vector", (0.0, -1.0, 0.0))
    dot = sum(a * b for a, b in zip(v0, v1))
    mag0 = math.sqrt(sum(a * a for a in v0))
    mag1 = math.sqrt(sum(a * a for a in v1))
    arg = max(
        -1.0, min(1.0, dot / (mag0 * mag1))
    )  # 0.9 + 0.1 = 1.0000001 thats pretty based.... -> math.acos(1.0000001) is apparently illegal
...
def _raw_to_vector(raw_vector, defaults={}):
    gx, gy, gz = (
        axis / 1.0 / (defaults.get("base_g", 1.0) * FREEFALL_MS2) for axis in raw_vector
    )
    return (gx, gy, gz)
...
def _average_samples(samples: Sequence[Tuple[float, float, float]], amount: int = 0):
    ...
    window = (
        samples[:amount] if amount > 0 else samples[amount:] if amount < 0 else samples
    )
...
class ToolDropDetection:
    def __init__(self, config):
        ...
        self.gcode_macro = gcode_macro = self.printer.load_object(config, "gcode_macro")
        ...
        self.def_freq = config.getfloat(
            "polling_freq", 1.00, minval=0.01, maxval=MAX_POLL_FREQ
        )
        req_rate = config.getint("polling_rate", 10)  # checked/adjusted later

        # ────| crash detection stuff (disabled if not set)
        self.peak_g_thr = config.getfloat("peak_g_threshold", None, minval=0.01)
        self.rot_threshold = config.getfloat(
            "rotation_threshold", None, minval=0.0, maxval=180.0
        )
        self.pitch_threshold = config.getfloat(
            "pitch_threshold", None, minval=0.0, maxval=180.0
        )
        self.roll_threshold = config.getfloat(
            "roll_threshold", None, minval=0.0, maxval=180.0
        )

        self.drop_mintime = config.getfloat("drop_mintime", 1.0, minval=0.0)
        self.drop_template = gcode_macro.load_template(config, "crash_gcode", "")

        # ────| angle checking stuff
        self.hysteresis = config.getfloat(
            "angle_hysteresis", 5.0, minval=0.1, maxval=180.0
        )
        self.angle_exceed_template = self.gcode_macro.load_template(
            config, "angle_exceed_gcode", ""
        )
        self.angle_return_template = self.gcode_macro.load_template(
            config, "angle_return_gcode", ""
        )

        # ────| secondary
        self.decimals = config.getint("decimals", 3, minval=0, maxval=10)

        self.session_time = config.getfloat(
            "session_time", 1.00, minval=0.01, maxval=60
        )
        self.current_samples = config.getint("current_samples", 10, minval=0)
        ...
        _STATISTIC_FN = config.getchoice("samples_result", statistics_mode, "median")
        # ────| ensure valid if not errored before.
        self.def_rate = min(_RATE_CHOICES, key=lambda x: abs(x - req_rate))
        if self.def_rate != req_rate:
            self.startup_report += (
                f"[tool_drop_detection] polling_rate {req_rate} Hz is not "
                f"supported; using closest {self.def_rate} Hz instead."
            )

        # ── build two-way name maps ───────────────────────────────────────────────
        static_acc    = [n.strip() for line in raw_accelerometers for n in str(line).split(',') if n.strip()]
        discovered    = self._discover_tool_accelerometers(config, tool_param_key)

        seen = set()
        raw_acc = [x for x in static_acc + discovered if not (x in seen or seen.add(x))]

        if not raw_acc:
            raise config.error("tool_drop_detection: no accelerometers found — set 'accelerometer' "
                            "and/or 'params_accel' on at least one [tool ...] section.")

        self._build_name_mappings(raw_acc, default_accel_type)
        ...
        for short in self.full_to_short.values():
            raw_def = config.get(f"default_{short}", None)
        ...
        self.gcode.register_command(
            "TDD_POLLING_START",
            self._cmd_polling_start,
            desc=self.cmd_TDD_POLLING_START_help,
        )
        self.gcode.register_command(
            "TDD_POLLING_STOP",
            self._cmd_polling_stop,
            desc=self.cmd_TDD_POLLING_STOP_help,
        )
        self.gcode.register_command(
            "TDD_POLLING_RESET",
            self._cmd_polling_reset,
            desc=self.cmd_TDD_POLLING_RESET_help,
        )
        self.gcode.register_command(
            "TDD_QUERY", self._cmd_query, desc=self.cmd_TDD_QUERY_help
        )
        self.gcode.register_command(
            "TDD_REFERENCE_DUMP",
            self._cmd_dump_reference,
            desc=self.cmd_TDD_REFERENCE_DUMP_help,
        )
        self.gcode.register_command(
            "TDD_REFERENCE_SET",
            self._cmd_set_reference,
            desc=self.cmd_TDD_REFERENCE_SET_help,
        )
        self.gcode.register_command(
            "TDD_REFERENCE_RESET",
            self._cmd_reset_reference,
            desc=self.cmd_TDD_REFERENCE_RESET_help,
        )
        self.gcode.register_command(
            "TDD_START", self._cmd_start_crash_detect, desc=self.cmd_TDD_START_help
        )
        self.gcode.register_command(
            "TDD_STOP", self._cmd_stop_crash_detect, desc=self.cmd_TDD_STOP_help
        )

    def _discover_tool_accelerometers(self, config, key):
        found = []
        for section in config.get_prefix_sections('tool '):
            name = section if isinstance(section, str) else section.get_name()
            tool_obj = self.printer.load_object(config, name)
            val = getattr(tool_obj, 'params', {}).get(key)   # key = 'params_accel'
            if val:
                found.append(str(val).strip())
        return found

    def _build_name_mappings(self, raw_acc, default_type):
        ...
        for raw in raw_acc:
            ...
            if len(parts) == 2:
                ...
                short = raw
                full  = f"{default_type} {short}"
            self.name_to_full[raw] = full
            self.name_to_full[full] = full
            self.name_to_full[short] = full
    ...
    def _on_home_begin(self, *_):
        ...
        for (
            p
        ) in self.pollers.values():  # fast flush so we don’t accumulate unread samples
    ...
    def _targets(self, gcmd) -> List[str]:
        ...
        requested, unknown = [], []
        for tok in (t.strip() for t in acc.split(',') if t.strip()):
            full  = self.name_to_full.get(tok)
            short = self.full_to_short.get(full) if full else None
            if short is None and tok in self.readers:
                short = tok
            (requested if short else unknown).append(short or tok)
        if unknown:
        ...
        for full in set(self.name_to_full.values()):
            ...
            if chip is None:
                raise self.config.error(
                    f"tool_drop_detection: cannot find accelerometer section '{full}'"
                )
    ...
    def _cmd_start_crash_detect(self, gcmd):
        ...
        if (
            g_limit is None
            and angle_limit is None
            and pitch_limit is None
            and roll_limit is None
        ):
            raise gcmd.error(
                "No crash-detection limits supplied and nothing configured"
            )
    ...
    cmd_TDD_REFERENCE_RESET_help = (
        "[ACCEL] - Reset reference baseline to config defaults"
    )
    ...
    cmd_TDD_REFERENCE_DUMP_help = (
        "[ACCEL] - Dump current reference baseline data to console for copying"
    )
    ...
    def _cmd_dump_reference(self, gcmd):
        for short in self._targets(gcmd) or self._data:
            ...
            gcmd.respond_info(
                f"default_{short}: [g:{bg}  p:{bp}°  r:{br}°  vec:({vx},{vy},{vz})]"
            )
        ...
        for short in targets:
            ...
            if poll:
                ...
                tup_samples = (
                    ...
                    if raw and hasattr(raw[0], "accel_x")
                    else raw
                )
            else:
...
def load_config(config):
    return ToolDropDetection(config)
...
class _Reader:
        ...
        if self._chip is None:
            self._chip = (
                ...
                or self.printer.lookup_object(
                    f"adxl345 {self.name}", default=None
                )  # dunno which one works too lazy to check
            )
        return self._chip
    ...
    def grab(self, dwell: float = _DWELL_GRAB):  # -> None | tuple[Any, Any, Any]:
        ...
        return None if not sam else (sam[-1].accel_x, sam[-1].accel_y, sam[-1].accel_z)
...
class _Poller:
    def __init__(self, parent, name: str, freq: float, rate: int):
        ...
        self.xyz_history: Deque[Tuple[float, float, float]] = collections.deque(
            maxlen=max(1, int(parent.session_time * freq))
        )

        # ensure correct BW_RATE
        self.prev_rate = getattr(self.chip, "data_rate", rate)
        if rate != self.prev_rate:
            if self.full.startswith('adxl345 '):
                self.chip.set_reg(0x2C, adxl345.QUERY_RATES[rate])
                self.chip.data_rate = rate
            else:
                self.parent.gcode.respond_info(
                    f"[{self.full}] runtime RATE change not supported for this chip type; "
                    f"edit its 'rate:' in printer.config instead")

        self.angle_limit, self.pitch_limit, self.roll_limit, self.g_limit = (
            None,
            None,
            None,
            None,
    ...
    def _update_current(self: _Poller, avrg_xyz: Tuple[float, float, float]):
        ...
        current["rotation"] = {
            "pitch": round(pitch, self.dec),
            "roll": round(roll, self.dec),
            "vector": round(angle, self.dec),
        }
    ...
    def _update_session(self, xyz_samples):
        ...
        raw_mags = (
            _vector_to_magnitude(_raw_to_vector(v, self.defaults)) for v in xyz_samples
        )
    ...
    def _tick(self, ev):
        ...
        try:
            ...
            self.helper.request_start_time = (
                self.helper.request_end_time
            )  # tells it "give up on previous"
        ...
        while (
            self.helper.msgs and self.helper.msgs[0]["data"][-1][0] < sample_ts
        ):  # todo check if this actually needed? cant hurt i think. (prob very needed)
        ...
        self._update_current(
            current_average
        )  # ──[ current ones, always fairly up to date.

        cur_peak = max(
            _vector_to_magnitude(_raw_to_vector(v, self.defaults)) for v in xyz_samples
        )
        template_context = self.parent._build_context(
            self.name, cur_pitch, cur_roll, cur_vector_angle, cur_mag, cur_peak
        )

        self._check_angle_exceed(
            cur_vector_angle, cur_pitch, cur_roll, template_context
        )
        self._check_drop(
            cur_vector_angle, cur_pitch, cur_roll, cur_peak, template_context
        )
        ...
        if now - ev > self.period * 1.2:  # 20% too late!
            ...
            if self.overrun >= 5:  # five consecutive overruns -> chillax
                ...
                self.parent.gcode.respond_info(
                    f"oops, dont die on me! (reactor too busy) throttled {self.full} to {1 / self.period:.1f} Hz)"
                )
        else:
            self.overrun = 0
